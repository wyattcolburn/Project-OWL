...
void publish(redisContext* redisConnect, const char* stream_name, const char* key, const char* value, char* response) {
    ...
    // Construct the XADD command
    std::string command = "XADD " + std::string(stream_name) + " * " + std::string(key) + " " + std::string(value);

    // Send the command to Redis
    redisReply* reply = (redisReply*)redisCommand(redisConnect, command.c_str());
    ...
    if (reply->type == REDIS_REPLY_ERROR) {
        ...
        std::strcpy(response, reply->str);
        std::cout << "Message added with ID: " << reply->str << std::endl;
    } else if (reply->type == REDIS_REPLY_NIL) {
        std::cout << "No response from Redis" << std::endl;
    } else {
